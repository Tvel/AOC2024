...
{
    [Theory]
    [InlineData(Inputs.Sample, 143)]
    [InlineData(Inputs.Input, 4766)]
    public void Part1(string inputString, long expected)
    {
        ReadOnlySpan<char> input = inputString.AsSpan();
        (List<(int key, int value)> rulesList, ILookup<int, int> rulesLookup, List<List<int>> pages) = Parse(input);

        List<List<int>> correctPages = [];
        foreach (List<int> page in pages)
        {
            bool pageIsCorrect = true;
            for (int i = 0; i < page.Count; i++)
            {
                int ruleKey = page[i];

                if (rulesLookup[ruleKey]
                    .Select(r => page.IndexOf(r))
                    .Where(indexOf => indexOf != -1)
                    .Any(indexOf => indexOf < i))
                {
                    pageIsCorrect = false;
                }

                if (!pageIsCorrect) break;
            }
    ...
    [Theory]
    [InlineData(Inputs.Sample, 123)]
    [InlineData(Inputs.Input, 6257)]
    public void Part2(string inputString, long expected)
    {
        ReadOnlySpan<char> input = inputString.AsSpan();
        (List<(int key, int value)> rulesList, ILookup<int, int> rulesLookup, List<List<int>> pages) = Parse(input);

        List<List<int>> wrongPages = [];
        foreach (List<int> page in pages)
        {
            bool pageIsCorrect = true;
            for (int i = 0; i < page.Count; i++)
        ...
        int result = wrongPages.Select(page => page
                .Select(n => (
    ...
    private static (List<(int key, int value)>, ILookup<int, int>, List<List<int>>) Parse(ReadOnlySpan<char> input)
    {
        List<(int key, int value)> rulesList = [];
        List<List<int>> pages = [];
        bool parseRules = true;
        Span<Range> rulesRange = stackalloc Range[2];
        foreach (var rowRange in input.Split(Environment.NewLine))
        {
            ...
            if (parseRules)
            {
                row.Split(rulesRange, '|');
                rulesList.Add((int.Parse(row[rulesRange[0]]), int.Parse(row[rulesRange[1]])));
            }
            else
            {
                List<int> page = [];
                foreach (var pageRange in row.Split(','))
                {
                    page.Add(int.Parse(row[pageRange]));
                }
                pages.Add(page);
            }
        }

        ILookup<int, int> lookup = rulesList.ToLookup(x => x.key, x => x.value);

        return (rulesList, lookup, pages);
    }
}
