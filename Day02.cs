...
{
    [Theory]
    [InlineData(Inputs.Sample, 2)]
    [InlineData(Inputs.Input, 252)]
    public void Part1(string inputString, long expected)
    {
        List<List<int>> reports = ParseReports(inputString);

        // ReSharper disable once ReplaceWithSingleCallToCount
        int result = reports
            .Select(r => r.Zip(r.Skip(1)))
            .Where(x => x.Select(y => Math.Abs(y.First - y.Second)).All(y => y is < 4 and > 0))
            .Where(x => x.Select(y => y.First > y.Second).Distinct().Count() == 1)
            .Count();
    ...
    [Theory]
    [InlineData(Inputs.Sample, 4)]
    [InlineData(Inputs.Input, 324)]
    public void Part2(string inputString, long expected)
    {
        List<List<int>> reports = ParseReports(inputString);

        int result = reports
            .Select(x => Enumerable.Range(0, x.Count).Select(y => x.Where((_, i) => i != y).ToList()))
            .Count(r => r.Select(x => x.Zip(x.Skip(1)))
                .Where(x => x.Select(y => Math.Abs(y.First - y.Second)).All(y => y is < 4 and > 0))
                .Where(x => x.Select(y => y.First > y.Second).Distinct().Count() == 1)
                .Any()
            );
    ...
    private static List<List<int>> ParseReports(string inputString)
    {
        ...
        foreach (var rowRange in input.Split(Environment.NewLine))
        {
            ...
            foreach (Range levelRange in row.Split(' '))
            {
                int level = int.Parse(row[levelRange]);
                report.Add(level);
            }
            reports.Add(report);
        }
